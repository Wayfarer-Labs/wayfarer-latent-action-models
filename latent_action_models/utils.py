...
def colors_labels_from_actions(
    actions_batch: List[Union[pl.DataFrame, Literal['NO_ACTION']]],
    full_actions_df: pl.DataFrame,
    num_frames_per_clip: int,
    *,
    top_n_common: int = 8
) -> Tuple[List[Tuple[int, int, int]], List[str], Dict[str, Tuple[int, int, int]]]: # MODIFIED: Return signature
    """
    ...
    output_colors = []
    output_labels = [] # NEW: Create a parallel list for string labels
    ...
    for clip_data in actions_batch:
        if isinstance(clip_data, pl.DataFrame):
            ...
            for row in clip_with_labels.to_dicts():
                ...
                if combo in common_labels_list:
                    label = combo.upper()
                elif combo == "":
                    label = "IDLE"
                else:
    ...
    return output_colors, output_labels, legend_map # MODIFIED: Return all three
